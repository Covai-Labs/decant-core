...
export class JoylandParser extends ChatParser {
  ...
  async parse() {
    ...
    const botNameEl =
      document.querySelector('.bot-name-text') || document.querySelector('.bot-name');
    let botName = botNameEl ? botNameEl.textContent.trim().replace(/\s+/g, ' ') : '';
    if (botName.endsWith('1') && botNameEl?.parentElement?.querySelector('.bot-name-text')) {
    ...
    const creator = creatorEl ? creatorEl.textContent.trim().replace(/\s+/g, ' ') : '';
    ...
    for (const item of items) {
      ...
      const rawContentEl =
        item.querySelector('.markdown-body') || item.querySelector('.body-text') || item;
